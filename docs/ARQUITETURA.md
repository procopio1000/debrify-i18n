...
- localized option/display adapters
- runtime surface coverage ledger
...
They do not receive BuildContext, do not persist localized strings and do not expose PRODUCT_COPY final through getters como `formatted*`, `*Text`, `label`, `description` ou `displayName` quando essa copy pertence à apresentação.

Presentation mappers convert stable states to AppLocalizations. Brand/external/user data pode continuar em models, mas precisa de ownership explícito.
...
# Contratos adicionados na V10

## Declarative presentation dataflow

Copy app-owned armazenada em enum constructor, extension getter, model/service field, record ou registry continua sendo presentation copy. O inventory rastreia origem → consumidor/sink em vez de considerar seguro apenas porque o literal não está dentro de `Text(...)`.

Options persistidas mantêm stable id/storage key; um localized mapper separado fornece label/description para tela, Settings Search e accessibility.

## Temporal/compact boundary

Relógio civil e compact numbers não são materializados em domain/services. Casos baseline como `CalendarTimeFormat`, `StremioTvNowPlaying.progressText`, Debrify TV stats e `formattedViews/formattedScore` são migrados para presentation formatters com locale explícito.

## Coverage closure

Para a tree auditada:
...
Gate Q bloqueia qualquer path runtime relevante que não pertença exatamente a uma das classes. A evidência inclui path/blob SHA/reachability/ownership/source kind/scanner/status.
